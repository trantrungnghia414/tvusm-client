...
import { News, NewsStatsType } from "./types/newsTypes";
...
export default function NewsPage() {
        ...
        try {
            ...
            const statsData: NewsStatsType = {
                ...
                draftNews: data.filter((item: News) => item.status === "draft")
                    .length,
                featuredNews: data.filter((item: News) => item.is_featured)
                    .length,
            };
            setStats(statsData);
        } catch (error) {
    ...
    // Lấy danh sách tin tức khi component mount
    useEffect(() => {
        fetchNews();
    }, [fetchNews]);
    ...
    // Điều hướng đến trang xem chi tiết tin tức
